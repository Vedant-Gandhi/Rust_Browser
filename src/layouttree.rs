...
#[derive(Clone)]
pub enum BoxType{
    Block,InlineBlock,Inline,Anonymous
}
impl <'a> LayoutContainer<'a>{
    pub fn new(boxtype:BoxType,style_node:& 'a StyleNode)->LayoutContainer<'a>{
        LayoutContainer{
            boxtype,dims:Default::default(),style_node,children_nodes:Vec::new()
        }
    }
    ...
    fn calculate_inline_vertical(&mut self,dims:Dimensions){
        ...
        d.coordinates.x=dims.coordinates.x+d.current.x+d.margin.left+d.padding.left+d.border.left;
        d.coordinates.y=dims.coordinates.y+d.current.y+d.border.top+d.margin.top+d.padding.top;
    }
    ...
    fn calculate_horizontal(&mut self,dims:Dimensions){
        ...
        let total=width+left_margin+right_margin+d.border.left+d.border.right+d.padding.right+d.padding.left;
        ...
        for single_child in &mut self.children_nodes{
           ...
           let new_height= single_child.dims.get_margin_dimensions().width;
            ...
            match single_child.boxtype{
                BoxType::Block=>dims.coordinates.height+=single_child.dims.get_margin_dimensions().height,
                BoxType::InlineBlock=>{
    ...
    fn calculate_height(&mut self){
        self.style_node.get_value("height").map_or((),|h| match **h{
...
impl Rectangle{
    fn expand(&self,e:EdgeValues)->Rectangle{
        Rectangle{x:self.x-e.left,y:self.y-e.top,width:self.width+e.left+e.right,height:self.height+e.top+e.bottom}
    }
}
...
impl fmt::Debug for EdgeValues{
    fn fmt(&self,f:&mut fmt::Formatter)->fmt::Result{
        write!(f,"Left:{:?} \n Top:{:?}\n Right:{:?}\n Bottom:{:?}",self.left,self.top,self.right,self.bottom)
    }
}
...
pub fn get_layout_tree<'a>(root: &'a StyleNode<'a>,mut viewport_dims:Dimensions)->LayoutContainer<'a>{
...
fn generate_layout_tree<'a>(node:&'a StyleNode)->LayoutContainer<'a>{
    let mut layout_node=LayoutContainer::new(match node.get_display_value(){
        Display::Block=>BoxType::Block,Display::Inline=>BoxType::Inline,Display::InlineBlock=>BoxType::InlineBlock,Display::None=>BoxType::Anonymous
    }, node);
