...
fn render_background(commands:&mut DisplayCommandList,root:&LayoutContainer){
    get_background_color(root,"background-color").map(|color|{
        commands.push(DisplayCommand::SolidRect(color,root.dims.get_border_dimensions()))
    });   
}
...
fn render_borders(commands:&mut DisplayCommandList,root:&LayoutContainer){
    ...
    commands.push(DisplayCommand::SolidRect(color.clone(),Rectangle{x:root_border_dims.x,y:root_border_dims.y,width:root_dims.border.left,height:root_border_dims.height}));
    commands.push(DisplayCommand::SolidRect(color.clone(),
    Rectangle
    {
...
impl fmt::Debug for DisplayCommand{
    fn fmt(&self,f:&mut fmt::Formatter)->fmt::Result{
        match *self{
            DisplayCommand::SolidRect(ref c ,ref rect)=>write!(f,"Color:{:?}\n Rectangle :{:?}",c,rect),
        }
    }
}